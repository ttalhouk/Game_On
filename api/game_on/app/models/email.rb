...
class Email
  ...
  def initialize
    @client = SendGrid::Client.new(api_key: 'GAME_ON_SENDGRID')
  end
  ...
  def send_game_on_email(home_team, away_team, game)
    ...
    players.each do |player|
      mail = SendGrid::Mail.new do |m|
        ...
        m.html = "Hello #{player.name}, /n Game is On! The game is at #{game.start_time}. /n This will be at #{game.address}, #{game.city}, #{game.zip}."
      end
      res = @client.send(mail)
      puts res.code
      puts res.body
